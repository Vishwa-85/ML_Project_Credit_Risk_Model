...
MODEL_PATH = 'artifacts_vishwa/model_data.joblib'
...
def prepare_input(age, income, loan_amount, loan_tenure_months, avg_dpd_per_delinquency,
    ...
    input_data = {
        ...
        'Avg_DPD_Delinquent_months_ratio': avg_dpd_per_delinquency,
...
def calculate_credit_score(input_df, base_score=300, scale_length=600):
    # The following equation is: y = mx+C in which m is model coefficient, x is input values, c is model intercept, transposed (hence T)
    ...
    # flattening because we dont want an array, just a single number
    # Converting to credit score is business defined, so we can consider the following formula
    credit_score = base_score + non_default_probability.flatten() * scale_length

    # Determine the rating category based on the credit score (Business defined)
